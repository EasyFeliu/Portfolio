...
import { SECTION_IDS, copy, navLinks, profile, sectionHref } from "@/lib/content";
    ...
    <header className="fixed inset-x-0 top-0 z-50">
      ...
      >
        <div className="container-page flex h-[var(--header-h)] items-center justify-between gap-4">
          ...
          >
            {profile.shortName}
            <span className="sr-only"> — {copy.nav.homeHint}</span>
          </a>

          <nav aria-label={copy.nav.mainLabel} className="hidden md:block">
          ...
          <div className="flex items-center gap-2">
            ...
            <a
              ...
              className="btn btn-primary hidden min-h-9 px-4 py-2 text-sm md:inline-flex"
            >
              {copy.nav.cta}
            </a>
            <button
      ...
      <div
        ...
        role="dialog"
        aria-modal="true"
        aria-label={copy.nav.dialogLabel}
        hidden={!menuOpen}
        className="glass-bar h-[calc(100dvh-var(--header-h))] overflow-y-auto border-b border-hairline md:hidden"
      >
        <nav aria-label={copy.nav.mobileLabel} className="container-page py-4">
